...
冒頭で記載した通り、本教材では新たに追加するコードに対してのみ webpack を利用するようにしていきます。そのため、Rails チュートリアルで作成した既存の Javascript が格納されたファイルとは別に`/app/javascript/packs`フォルダを作成し、配下に`application.js`を作成します。Webpack の設定では、ここをエントリーポイントにして進められるようにしておきましょう。また、以降の資料ではここを作業ディレクトリとして置き換えて読み進めるようにしてください。
