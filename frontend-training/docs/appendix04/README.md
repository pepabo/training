# 付録 4 Rails フロントエンド環境セットアップ(Rails 6 以前)
...
ところでみなさんは Rails Tutorial 第 8 章で `yarn add jquery@3.4.1 bootstrap@3.4.1` というコマンドを打ったことを覚えていますか。その結果 `package.json` というファイルの `dependencies` に `jquery` と `bootstrap` が追加されたことがわかったと思います。
...
1. ここまでできたら、 [webpack-cli で利用可能なコマンド](https://webpack.js.org/api/cli/) を見て、試しにバンドルファイルをビルドしてみましょう。もちろんコマンドには `npx` を先頭につけることをお忘れなく。 `public/packs` ディレクトリに `main.ce5b5fc52a137b3fd42e.js` のようなファイルができていれば成功です。
...
ついに本題であるところの [React](https://ja.reactjs.org/) を導入し、本格的なフロントエンド開発に乗り出します。 Vue や React の何が jQuery などの他のフロントエンドライブラリと比べて優れているかについて知ることができます。18 章に進んでください。
